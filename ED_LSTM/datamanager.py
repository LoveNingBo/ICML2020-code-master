...
class DataManager(object):
    def __init__(self, dataset):
        ...
        for fname in ['train', 'valid', 'test']:
            ...
            self.origin[fname] = data
        self.slot={}
        for line in open('%s/vocab.slot' % (dataset)):
            s = line.strip()
            self.slot[s]=len(self.slot)
    """
    获取单词字典
    """
        ...
        for fname in ['train', 'valid', 'test']:
            ...
            for sent in self.origin[fname]:
                ...
                if lens>temp_max_length:
                    temp_max_length=lens
                words += [0] * (maxlenth - lens)
                solution =[ one_hot_vector(int(self.slot[slot_name])) for slot_name in slots]
                # print(words)
                now = {'words': np.array(words), \
                       'solution': solution, \
                       'lenth': lens}
